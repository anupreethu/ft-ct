  ...
  methods: {
    ...
    async getMatchUser(email) {
      try {
        ...
        // Handle errors, for example, log them or show an error message
        console.error(error);
        this.matchUser = null; // Set matchUser to null if there was an error
      }
    },
    ...
    async handleSubmit(e) {
      ...
      if (this.errors.length === 0) {
        ...
        } else if (this.matchUser.user_password === this.loginObj.pass) {
          this.matchUser.user_password = "";
          this.setUser(this.matchUser);
          this.$router.push("/menu"); // Redirect to the menu page
        } else {
