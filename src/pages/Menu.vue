            ...
            <div class="col-sm-4 col-12 filter-box">
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        ...
                        <li>
                            <input type="button" name="cbStatus" id="sdStatus" value="Seasonal paints" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="sdStatus" class="d-flex justify-content-between">Seasonal paints <button
                                    class="unselect-btn" @click="unselectStatusBtn($event)"
                                    value="Seasonal paints">X</button></label>
                        </li>


                        <li>
                            <input type="button" name="cbStatus" id="ndStatus" value="New paints" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="ndStatus" class="d-flex justify-content-between">New paints <button
                                    class="unselect-btn" @click="unselectStatusBtn($event)"
                                    value="New paints">X</button></label>
                        </li>
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            <input type="button" name="rPrice" id="rtfPrice" value="1398,1400" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rtfPrice" class="d-flex justify-content-between">₹1398 - ₹1400 <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="rftPrice" value="1400,5000" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rftPrice" class="d-flex justify-content-between">₹1400 - ₹5000 <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="rttPrice" value="5000,7000" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rttPrice" class="d-flex justify-content-between">₹5000 - ₹7000<button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="mtPrice" value="1200" hidden
                                @click="filterPriceBtn($event)" />
                        ...
                        <li>
                            <input type="button" name="rPrice" id="ltPrice" value="698" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="ltPrice" class="d-flex justify-content-between">{{ "<" }} ₹698 <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>
                ...
                <div class="row filter-heading">
                    <h1>"Unlock the magic of color with every brushstroke from our paint box."</h1>
                </div>

                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            <input type="button" name="rType" id="mType" value="Brush your dreams to life." hidden
                                @click="filterTypeBtn($event)" />
                            <label for="mType" class="d-flex justify-content-between">Brush your dreams to life<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rType" id="vType" value="A world of hues in every box" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="vType" class="d-flex justify-content-between">A world of hues in every box<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>
...
export default {
    ...
    methods: {
        ...
        checkSale: function (food, statusArray) {
            if (statusArray.includes("Sale on")) {
        ...
        checkOnl: function (food, statusArray) {
            if (statusArray.includes("Online Only")) {
                if (food.food_status.includes("newly arrived")) {
                    return true;
                }
                else {
                    return false;
        ...
        checkSeason: function (food, statusArray) {
            if (statusArray.includes("Seasonal paints")) {
                if (food.food_status.includes("seasonal paints")) {
                    return true;
                }
                else {
                    return false;
        ...
        checkNew: function (food, statusArray) {
            if (statusArray.includes("New paints")) {
                if (food.food_status.includes("new paints")) {
                    return true;
                }
                else {
                    return false;
        ...
        evaluatePrice: function (food, priceRange) {
            ...
            if (priceRange == "1398,1400") {
                if (1398 <= cal && cal <= 1400) {
                    return food;
                }
            }
            else if (priceRange == "1400,5000") {
                if (1400 <= cal && cal <= 5000) {
                    return food;
                }
            }
            else if (priceRange == "5000,7000") {
                if (5000 <= cal && cal <= 7000) {
                    return food;
                }
            }
